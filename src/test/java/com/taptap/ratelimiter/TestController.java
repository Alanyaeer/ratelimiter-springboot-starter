...
public class TestController {
    ...
    @GetMapping("/get")
    @RateLimit(rate = 2, rateInterval = "10s", customKeyFunction = "keyFunction", fallbackFunction = "getFallback")
    public String get(@RateLimitKey("'Hello World'") String name) {
        return "get";
    }
    ...
    @PostMapping("/hello/slide-time-window-rate-limiter")
    @RateLimit(mode = Mode.SLIDE_WINDOW, rate = 5, rateInterval = "10s", keys = {"#user.name", "#user.id"})
